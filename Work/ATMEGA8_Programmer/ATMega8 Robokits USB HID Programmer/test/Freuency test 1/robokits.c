...
-----------------------------------------------------------------*/
//#define cnt0            0x00
#define TIMER_0_CNT     0xFC  // 26 micros sec, use AVRcalc to calculate these values
                                // ((TCNT0=0x63))
								// DECIMAL VALUE = 252
								// MAX VALUE = 255
								// TIMER OVERFLOW ON 255-252 = 3 COUNTS


#define TIMER_0_CNT1   0xFB		// 32 micro sec, use AVRcalc to calculate these values
                                // ((TCNT0=0x63))
								// DECIMAL VALUE = 193
								// MAX VALUE = 255
								// TIMER OVERFLOW ON 255-193 = 62 COUNTS
#define CNT             0xff
#define TMC8_CK1024	(_BV(CS01))	// 8 BIT TIMER0 AT 8 PRESCALAR
												// GIVES 8*62 = 496 CYCLES
												// AT 1MHZ FREQUENCY DELAY OF 0.496 milliSECONDS
...
{
	if(!bit_is_clear(PIND,6))
	{	TCNT0 = TIMER_0_CNT1-(TIMER_0_CNT-TIMER_0_CNT1);
			//BLINK LED3 ON TIMER INTERRUPT
		cbi(PORTD,6);
	}
	else
