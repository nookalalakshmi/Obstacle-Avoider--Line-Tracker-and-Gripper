...
function ch_write_iframe(f, thehtml, r) {
  var w = window;
...
function ch_mm() {
  ...
  w.onerror = w.ch_oeh;
  var amm_host = ch_def(w.ch_host, "amm.chitika.net");
  ...
  if (w.ch_search_referral) {
    if (!w.ch_allow_pixel && (w.ch_alternate_ad_js || w.ch_alternate_js_callback) && !String(w.ch_referrer).match(/(google|search.yahoo|search.msn|search.live|ask|search.aol).com/)) {
      ch_clear();
